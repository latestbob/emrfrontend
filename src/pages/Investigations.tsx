              ...
              <tbody className="text-gray-200">
                {serve.investigations.map((inv: any, idx: number) => (
                  <tr key={idx}>
                    ...
                    <td className="py-2">
                      {inv.billing_status == null && (
                        <span className="text-xs bg-amber-400 p-1 rounded text-black">
                          Awaiting Billing
                        </span>
                      )}
                      {inv.billing_status == "invoiced" && (
                        <span className="text-xs bg-blue-600  p-1 rounded">
                          Invoiced
                        </span>
                      )}

                      {inv.billing_status == "billed" && (
                        <span className="text-xs bg-green-600  p-1 rounded">
                          Billed
                        </span>
                      )}
                    </td>
                  ...
                  <tbody>
                    {encounters &&
                      encounters.map((serve, index) => {
                        const formattedDate = moment(serve.request_date).format(
                          "MM/DD/YYYY hh:mm A"
                        );
                        return (
                          <tr
                          ...
                          >
                            ...
                            <td className="px-6 py-4 w-72">
                              ...
                              <span className="text-cyan-900 text-sm">
                                {serve.patient?.fullname || "No Name"} |{" "}
                                <span className="text-xs">
                                  DOB : {serve.patient.dob}
                                </span>{" "}
                                |{" "}
                                <span className="text-xs">
                                  Sponsor : {serve.patient.sponsor}
                                </span>{" "}
                                |{" "}
                                <span className="text-xs">
                                  Plan : {serve.patient.sponsor_plan}
                                </span>
                              </span>
                            </td>
                            <td className="px-6 py-4 w-32 text-xs">
                            ...
                            <td className="px-6 py-4 w-24 text-xs">
                              {serve.isUrgent ? (
                            ...
                            <td className="px-6 py-4 w-72 text-sm">
                            ...
                            <td className="px-6 py-4 w-64 text-xs">
                              ...
                              {serve.otherservices.length > 0 && (
                                <span
                                  onClick={() => handleOthersModalChange(index)}
                                  className="text-xs bg-amber-600 text-white mt-55 px-1 rounded cursor-pointer"
                                >
                                  has other services
                                </span>
                              )}
...

{
                  ...
                  {/* Render the modal outside the table */}
                  {renderImagingModal()}
                  {renderOthersModal()}
                </table>
              </div>
